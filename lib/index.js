...
var reader = {
  init:function(filename){
    ...
    try{
      this.httppath = require(p+'/build.js').onlineHttpPath;
    }catch(e){
      throw('未找到build.js文件');
    }
  ...
  comboFiles:function(pathArr,cb){
    ...
    pathArr.forEach(function(p,k){
      ...
      fs.readFile(p,function(err,data){
        if(err) throw err;
        comboArr[k] = self.fixModName(self.clearBlankStr(data.toString()),p);
        counter--;
        if(counter<=0){
          cb && cb(comboArr.join(';'));
        }
  ...
  fixModName:function(s,p){
      ...
      if(s.slice(i+len,i+len+8) == 'function'){
        var modname = p.replace(this.filepath,this.httppath);
